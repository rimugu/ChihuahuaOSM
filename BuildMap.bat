...
rem #  -----    ----------   ---------------------------------------------         #
rem #  N/A      2023-12-05   First Github version                                  #
rem ################################################################################
...
goto get_data_section_3
rem goto join_section
rem goto split_section
goto mkgmap_section

rem curl --parallel --keepalive-time 5 --output C:\temp\DirectChihuahuaOSM\input\ChihuahuaOSM46.osm --url "http://overpass.openstreetmap.ru/cgi/xapi_meta?*%5Bbbox=-106.914,30.354,-106.104,31.342%%5D"
...
cmd /c %osmosis_DIR%osmosis.bat^
 ...
 --bounding-box top=34.304 left=-109.344 bottom=25.414 right=-102.053 --write-xml %osmosis_temp%Chihuahua.osm
...
@echo on
java -jar %mkgmap_DIR%mkgmap.jar --verbose --max-jobs=5 --keep-going --family-id=6775 --product-id=1 --remove-short-arcs --net --route  --location-autofill=bounds,is_in,nearest^
 ...
 --bounds=%Home_DIR%bounds-latest.zip --output-dir=%Img_DIR% --gmapsupp --mapname=24740001 --area-name=ChihuahuaOSM --code-page=1252 --improve-overview --order-by-decreasing-area^
...
rem amenity=place_of_worship [0x6404 resolution 24]
...
rem natural=heath [0x4f resolution 24]
...
rem amenity=place_of_worship [0x13 resolution 24]
